...
if {$::tcl_version < 8.5} {
    proc ::xsdb::huddle::huddle {command args} {
        ...
        if {[info exists ::xsdb::huddle::methods($command)]} {
            return [eval $::xsdb::huddle::methods($command) $command $args]
        }
        return [eval ::xsdb::huddle::$command $args]
    }
    # some subcommands conflict reserved words. so, add prefix "_" (e.g. from "set" to "_set")
    proc ::xsdb::huddle::proc_add_ub {command args} {
        return [eval ::xsdb::huddle::_$command $args]
    }
} else {
    proc ::xsdb::huddle::huddle {command args} {
        variable ::xsdb::huddle::methods
        if {[info exists ::xsdb::huddle::methods($command)]} {
            return [$::xsdb::huddle::methods($command) $command {*}$args]
        }
        return [::xsdb::huddle::$command {*}$args]
    }

    proc ::xsdb::huddle::proc_add_ub {command args} {
        return [::xsdb::huddle::_$command {*}$args]
    }
}
...
proc ::xsdb::huddle::call {tag cmd arg} {
    variable types
    return [eval $types(callback:$tag) $cmd $arg]
}

proc ::xsdb::huddle::combine {args} {
    variable types
...
proc ::xsdb::huddle::to_node {src {tag ""}} {
    if {$tag eq ""} {set tag s}
    if {[isHuddle $src]} {
        return [lindex $src 1]
    } else {
...
proc ::xsdb::huddle::wrap {head src} {
    if {$head ne ""} {
...
proc ::xsdb::huddle::remove {src args} {
    checkHuddle $src
...
proc ::xsdb::huddle::equal {obj1 obj2} {
    checkHuddle $obj1
    checkHuddle $obj2
    return [_equal_subs [lindex $obj1 1] [lindex $obj2 1]]
}
proc ::xsdb::huddle::_equal_subs {obj1 obj2} {
    variable types
...
proc ::xsdb::huddle::_append {objvar args} {
    variable types
...
proc ::xsdb::huddle::_set_subs {command node len path value} {
    variable types
...
proc ::xsdb::huddle::_key_reflexive {command node len path {option ""}} {
    variable types
    foreach {tag src} $node break
    if {$len > 1} {
        set key [lindex $path 0]
...
proc ::xsdb::huddle::_get2 {node path strip} {
    variable types
    foreach {tag src} $node break
...
proc ::xsdb::huddle::_type {node nop nop} {
    variable types
    foreach {tag src} $node break
    return $types(type:$tag)
}

proc ::xsdb::huddle::_strip_wrap {head src {striped 0}} {
    if {$striped} {
        return [strip $src]
    } else {
...
proc ::xsdb::huddle::_dict_setting {command args} {
...
proc ::xsdb::huddle::_list_setting {command args} {
    switch -- $command {
        setting {
            return {
                type list
...
proc ::xsdb::huddle::_string_setting {command args} {
    switch -- $command {
        setting {
            return {
                type string
...
proc ::xsdb::huddle::jsondump {data {offset "  "} {newline "\n"} {begin ""}} {
    variable types
...
# etc..
proc ::xsdb::huddle::compile {spec data} {
    while [llength $spec] {
...
namespace eval ::xsdb::huddle {
    ...
    array set types {}
    array set callbacks {}
    ::xsdb::huddle::addType ::xsdb::huddle::_dict_setting
    ::xsdb::huddle::addType ::xsdb::huddle::_list_setting
    ::xsdb::huddle::addType ::xsdb::huddle::_string_setting
    set methods(set)    ::xsdb::huddle::proc_add_ub
    set methods(append) ::xsdb::huddle::proc_add_ub
