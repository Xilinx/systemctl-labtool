...
namespace eval ::xsdb::yaml {
    ...
    array set defaults {
        ...
        composer {
            !!binary ::xsdb::yaml::_composeBinary
        }
        parsers {
            timestamp ::xsdb::yaml::_parseTimestamp
        }
        shorthands {
            !! {tag:yaml.org,2002:}
        }
...
proc ::xsdb::yaml::yaml2dict {args} {
    ...
    return [::xsdb::huddle::huddle strip $result]
}
...
proc ::xsdb::yaml::setOptions {argv} {
    variable defaults
...
proc ::xsdb::yaml::list2yaml {list {indent 2} {wordwrap 40}} {
    return [huddle2yaml [eval ::xsdb::huddle::huddle list $list] $indent $wordwrap]
}

proc ::xsdb::yaml::dict2yaml {dict {indent 2} {wordwrap 40}} {
    return [huddle2yaml [eval ::xsdb::huddle::huddle create $dict] $indent $wordwrap]
}
...
proc ::xsdb::yaml::_getOption {argv} {
    variable data
    variable parsers
    variable fixed
    variable composer
...
proc ::xsdb::yaml::_imp_getOptions {{argvvar argv}} {
...
#########################
proc ::xsdb::yaml::_composeTags {tag value} {
    if {$tag eq ""} {return $value}
    set value [::xsdb::huddle::huddle strip $value]
    if {$tag eq "!!str"} {
        set pair [list $tag $value]
    } elseif {[info exists ::xsdb::yaml::composer($tag)]} {
        set pair [$::xsdb::yaml::composer($tag) $value]
    } else {
        error [_getErrorMessage TAG_NOT_FOUND $tag]
    }
    return  [eval ::xsdb::huddle::huddle wrap $pair]
}

proc ::xsdb::yaml::_composeBinary {value} {
    package require base64
    return [list !!binary [::base64::decode $value]]
}

proc ::xsdb::yaml::_composePlain {value} {
    if {$value ne ""} {
        if {[::xsdb::huddle::huddle type $value] ne "plain"} {return $value}
        set value [::xsdb::huddle::huddle strip $value]
    }
...
proc ::xsdb::yaml::_toType {value} {
    ...
    foreach {type} $::xsdb::yaml::data(types) {
        if {[info exists ::xsdb::yaml::parsers($type)]} {
            set pair [$::xsdb::yaml::parsers($type) $value]
        ...
        switch -- $type {
            ...
            default {
                # !!null !!true !!false
                if {[info exists ::xsdb::yaml::fixed($type:Group)] \
                 && [lsearch $::xsdb::yaml::fixed($type:Group) $lowerval] >= 0} {
                    set value $::xsdb::yaml::fixed($type:Value)
...
# Block Node parser
####################
proc ::xsdb::yaml::_parseBlockNode {{status ""} {indent -1}} {
    variable data
    set prev {}
    set result {}
    set scalar 0
...
#                 incr current
    ...
    if {$status eq "SEQUENCE"} {
        set result [eval ::xsdb::huddle::huddle sequence $result]
        ...
        if {![::xsdb::huddle::huddle isHuddle $result]} {
            set result [::xsdb::huddle::huddle wrap !!str $result]
        }
    }
    if {$tag ne ""} {
...
proc ::xsdb::yaml::_mergeExpandedAliases {result pos prev} {
    if {$result eq ""} {set result [::xsdb::huddle::huddle mapping]}
    if {$prev ne ""} {
    ...
    if {[::xsdb::huddle::huddle type $value] eq "list"} {
        set len [::xsdb::huddle::huddle llength $value]
        for {set i 0} {$i < $len} {incr i} {
            set sub [::xsdb::huddle::huddle get $value $i]
            set result [::xsdb::huddle::huddle combine $result $sub]
        }
        unset sub len
    } else {
        set result [::xsdb::huddle::huddle combine $result $value]
    }
    return [list $result $prev]
}


proc ::xsdb::yaml::_parseSubBlock {pos statusnew} {
...
proc ::xsdb::yaml::_set_huddle_mapping {result prev} {
    foreach {key val} $prev break
    set val [_composePlain $val]
    if {[::xsdb::huddle::huddle isHuddle $key]} {
        set key [::xsdb::huddle::huddle strip $key]
    }
    if {$result eq ""} {
        set result [::xsdb::huddle::huddle mapping $key $val]
    } else {
        ::xsdb::huddle::huddle append result $key $val
    }
    return $result
}
...
proc ::xsdb::yaml::_remove_duplication {dict} {
    array set tmp $dict
    array set tmp2 {}
...
proc ::xsdb::yaml::_parseBlockScalar {base separator} {
    ...
    return [::xsdb::huddle::huddle wrap !!str $value]
}

# in {> |}
proc ::xsdb::yaml::_parseBlockIndicator {} {
    set chomping "clip"
    set explicit 0
    while {1} {
        set type [_getc]
...
proc ::xsdb::yaml::_parsePlainScalarInBlock {base {loop 0}} {
    if {$loop == 5} { return }
    variable data
...
# Flow Node parser
####################
proc ::xsdb::yaml::_parseFlowNode {{status ""}} {
    set scalar 0
    set result {}
    set tag ""
    set prev {}
...
proc ::xsdb::yaml::_pushValue {result prev status value scope} {
    switch -- $status {
        "SEQUENCE" {
...
proc ::xsdb::yaml::_parseScalarNode {type scope {pos 0}} {
    set tag !!str
    switch -- $type {
    ...
    return [::xsdb::huddle::huddle wrap $tag $value]
}
...
proc ::xsdb::yaml::_parseTimestamp {scalar} {
...
proc ::xsdb::yaml::_parseDirective {} {
    variable data
    variable shorthands
...
proc ::xsdb::yaml::_parseDoubleQuoted {} {
...
proc ::xsdb::yaml::_parseSingleQuoted {} {
    set reStr {(?:(?:')(?:[^']*(?:''[^']*)*)(?:'))}
    set result [_getFoldedString $reStr]
...
proc ::xsdb::yaml::_parsePlainScalarInFlow {} {
    set sep {\t \n,\[\]\{\}}
...
# Generic parser
####################
proc ::xsdb::yaml::_getFoldedString {reStr} {
    variable data
...
proc ::xsdb::yaml::_skipSpaces {{commentSkip 0}} {
    variable data
...
# (cannot _ungetc)
proc ::xsdb::yaml::_getLine {{scrolled 1}} {
    variable data
...
proc ::xsdb::yaml::_getpos {} {
    variable data
    return $data(start)
}

proc ::xsdb::yaml::_setpos {pos} {
    variable data
    set data(start) $pos
}

proc ::xsdb::yaml::_ungetc {{len 1}} {
    variable data
...
proc ::xsdb::yaml::_setAnchor {anchor value} {
    variable data
    set data(anchor:$anchor) $value
}

proc ::xsdb::yaml::_getAnchor {anchor} {
    variable data
    if {![info exists data(anchor:$anchor)]} {error [_getErrorMessage ANCHOR_NOT_FOUND]}
    return  $data(anchor:$anchor)
}

proc ::xsdb::yaml::_getErrorMessage {ID {p1 ""}} {
    set num [_getLineNum]
    if {$p1 != ""} {
        return "line($num): [subst -nobackslashes -nocommands $::xsdb::yaml::errors($ID)]"
    } else {
        return "line($num): $::xsdb::yaml::errors($ID)"
    }
}

# Finds and returns the indentation of a YAML line
proc ::xsdb::yaml::_getIndent {line} {
...
proc ::xsdb::yaml::_imp_huddle2yaml {data {offset ""}} {
    set nextoff "$offset[string repeat { } $::xsdb::yaml::_dumpIndent]"
    switch -- [::xsdb::huddle::huddle type $data] {
        "string" {
            set data [::xsdb::huddle::huddle strip $data]
            return [_dumpScalar $data $offset]
        }
        "list" {
            set inner {}
            set len [::xsdb::huddle::huddle llength $data]
            for {set i 0} {$i < $len} {incr i} {
                set sub [::xsdb::huddle::huddle get $data $i]
                set sep [expr {[::xsdb::huddle::huddle type $sub] eq "string" ? " " : "\n"}]
        ...
        "dict" {
            set inner {}
            foreach {key} [::xsdb::huddle::huddle keys $data] {
                set sub [::xsdb::huddle::huddle get $data $key]
                set sep [expr {[::xsdb::huddle::huddle type $sub] eq "string" ? " " : "\n"}]
...
proc ::xsdb::yaml::_dumpScalar {value offset} {
    if {   [string first "\n" $value] >= 0
...
proc ::xsdb::yaml::_doLiteralBlock {value offset} {
...
proc ::xsdb::yaml::_doFolding {value offset} {
...
proc ::xsdb::yaml::_simple_justify {text width {wrap \n} {cut 0}} {
    set brk ""
...
proc ::xsdb::yaml::_huddle_mapping {command args} {
    switch -- $command {
        ...
        mapping { ; # $args: all arguments after "huddle mapping"
            if {[llength $args] % 2} {error {wrong # args: should be "::xsdb::huddle::huddle mapping ?key value ...?"}}
            set resultL {}
            foreach {key value} $args {
                lappend resultL $key [::xsdb::huddle::huddle to_node $value !!str]
            }
            return [::xsdb::huddle::huddle wrap !!map $resultL]
        }
        default { ; # devolving to default dict-callback
            return [::xsdb::huddle::huddle call D $command $args]
        }
    }
}

proc ::xsdb::yaml::_huddle_sequence {command args} {
    switch -- $command {
        setting { ; # type definition
            return {
                type list
        ...
        sequence {
            set resultL {}
            foreach {value} $args {
                lappend resultL [::xsdb::huddle::huddle to_node $value !!str]
            }
            return [::xsdb::huddle::huddle wrap !!seq $resultL]
        }
        default {
            return [::xsdb::huddle::huddle call L $command $args]
        }
    }
}

proc ::xsdb::yaml::_makeChildType {type tag} {
    set procname ::xsdb::yaml::_huddle_$type
    proc $procname {command args} [string map "@TYPE@ $type @TAG@ $tag" {
        switch -- $command {
            ...
            default {
                return [::xsdb::huddle::huddle call s $command $args]
            }
        }
    }]
    return $procname
...
::xsdb::huddle::huddle addType ::xsdb::yaml::_huddle_mapping
::xsdb::huddle::huddle addType ::xsdb::yaml::_huddle_sequence
::xsdb::huddle::huddle addType [::xsdb::yaml::_makeChildType string !!str]
::xsdb::huddle::huddle addType [::xsdb::yaml::_makeChildType string !!timestamp]
::xsdb::huddle::huddle addType [::xsdb::yaml::_makeChildType string !!float]
::xsdb::huddle::huddle addType [::xsdb::yaml::_makeChildType string !!int]
::xsdb::huddle::huddle addType [::xsdb::yaml::_makeChildType string !!null]
::xsdb::huddle::huddle addType [::xsdb::yaml::_makeChildType string !!true]
::xsdb::huddle::huddle addType [::xsdb::yaml::_makeChildType string !!false]
::xsdb::huddle::huddle addType [::xsdb::yaml::_makeChildType string !!binary]
::xsdb::huddle::huddle addType [::xsdb::yaml::_makeChildType plain !!plain]
